...
En todos los programas necesitamos que partes de nuestro código se repita. De hecho todos los programas/aplicaciones tienen una parte que llamamos bucle principal (main loop) que es donde el programa se pasa casi todo el tiempo.

Un bucle siempre está formado por un bloque de código que se repita y por una condición que decide si ha de repetirse una vez más o se termina.

Tenemos 2 formas de hacer un bucle en pyton:

* Bucle **while** cuando tenemos cláramente definida la condición que se debe cumplir para que se repita
...
[![Vídeo: Repetición de código con bucles en Python](https://img.youtube.com/vi/OkYf6p0TCSg/0.jpg)](https://youtu.be/OkYf6p0TCSg)

[Vídeo: Repetición de código con bucles en Python](https://youtu.be/OkYf6p0TCSg)


### Bucle **while**

Hemos dicho que está indicado cuando una condición nos dice que repetir algo. Podnremos después de la palabra reservada **while** la condición de repetición, seguida de ":" que marcarán el comienzo del bloque que se va a repetir, con la indentación adecuada.

En este primer ejemplo vamos a contar hasta 100, impriendo los números. Usaremos una variable que hará como contador, que imprimiremos e incrementaremos a cada paso. La condición será que ese contador sea menor o igual a 100

```python
# imprimimos los 100 numeros
contador = 1
while contador <= 100:
    print(contador)
    contador += 1
```
Vamos a modifiar ahora el ejemplo anterior para que muestre sólo los números que son múltiplos de 7 hasta el número 100, para lo que añadimos una sentencia condicional que sólo mostrará el valor de contador si es múltiplo de 7.

Vemos como hemos creado distintos bloques de código con la correspondiente indentación.

```python
# imprimos los multiplos de 7 hasta 100
contador = 1
while contador <= 100:
    if contador % 7 == 0:
        print(contador)
...
Vamos a hacer ahora un programa que se detenga cuando encuentre el primer múltiplo de 77. Por tanto añadimos dentro del bloque de l condición if del la sentencia **break**, que detendrá la ejecución.

Al tener dos maneras de terminar el bucle (por que ya no se cumpla la condicón del while y porque llamamos a break), necesitamos una manera de distingarlas. Para ello podemos añadir un bloque **else** que sólo se ejecutará si hemos salido por que ya no se cumplía la condición del while.
...
```
Podemos ver los diferentes comportamientos cambiando el valor del número hasta el que llegamos buscando el múltiplo de 77: llegando hasta 75 se mostrará la sentencia del else, mientras que llegamos más allá del 77 se mostrará la linea anterior al break.

[![Vídeo: Bucles While en Python](https://img.youtube.com/vi/NbMt62FLt_U/0.jpg)](https://youtu.be/NbMt62FLt_U)

[Vídeo: Bucles While en Python](https://youtu.be/NbMt62FLt_U)

### Bucles infinitos

En algunas ocasiones no sabemos realmente la condición para detener un bucle, en ese caso diremos que hacemos un bucle infinito y saldremos del mismo por medio de una sentenca **break**
...
Otra manera de hacer es utilizando una variable booleana en la condición, que valdrá True hasta que encontremos el número. De esta manera no tendremos que usar **break**
...
[![Vídeo: Deteniendo la ejecución de bucles While con break](https://img.youtube.com/vi/t0UrBSdGAag/0.jpg)](https://youtu.be/t0UrBSdGAag)


[Vídeo: Deteniendo la ejecución de bucles While con break](https://youtu.be/t0UrBSdGAag)


### Bucle for

Hemos dicho que el bucle **for** está pensado para recorrer conjuntos de cosas, como puden ser palabras en un frase o las letras de una palabra. 

Por ello vamos a empezar haciendo un programa que nos deletra una palabra
...
Tanto en el bucle **for** como el **while** tenemos un recuros para hacer que dentro de una iteración no se ejecuten algunas de las sentencias del bloque. Es la sentencia **continue** que pasará a la siguiente iteración saltándose las líneas del bloque que queden de esa iteración. 
...
Podemos usar también el bucle **for** cuando queremos iterar sobre números usando la función **range( )** (rango). Necesitaremos una variable que actue de contador que será la que vaya recorriendo el rango.
...
El programa para mostrar los numeros hasta 100 sería
...
[![Vídeo: Bucle For: iterando entre secuencias de caracteres o rangos (range) de números](https://img.youtube.com/vi/pCq66J1Tbgk/0.jpg)](https://youtu.be/pCq66J1Tbgk)

[Vídeo: Bucle For: iterando entre secuencias de caracteres o rangos (range) de números](https://youtu.be/pCq66J1Tbgk)


## Bucles anidados

En algunas ocasiones necesitamos usar 2 bucles, uno dentro de otro para recorrer todas las posibilidades, por ejemplo si quisiéramos escribir las tablas de multiplicar enteras, encesitaríamos un bucle que cambia con cada tabla (la 1, la del 2, ...) y otro que cambia con el número dentro de cada tabla
...
Vamos a ver en este ejemplo como podemos calcular a la vez si son múltiplos de 5 y de 7 usando un bucle **for**
...
Vamos a comprobar el resultado con el cálculo algegraico que sabemos que es N*(N+1)/2
...
[![Vídeo: Bucles for anidados en Python](https://img.youtube.com/vi/GxyT-g_jI-I/0.jpg)](https://youtu.be/GxyT-g_jI-I)

[Vídeo: Bucles for anidados en Python](https://youtu.be/GxyT-g_jI-I)
