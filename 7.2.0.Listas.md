...
* **clear()**: borra todos los elmentos de una lista.

* **insert(posicion,valor)**: inserta en *posicion* el valor indicado, desplazando los siguiente.
...
Tambien podemos ordenar una lista con la función **sorted**:
```python
sorted(list)
```
...
Uno de los usos más frecuentes de las listas es el recorrer sus elementos. Ya dijimos que  los bucles for están pensado para recorrer colecciones. Veamos como hacerlo una lista:
...
Este ejemplo nos muestra que podemos utilizar varias variables en mismo bucle for, algo que haremos con frecuencia.

### Creación dinámica de listas

En ocasiones necesitamos generar una lista a partir de otra, realizando una operación sobre sus elementos. Para ello Python incorpora una manera muy cómoda de hacerlo, donde expecificamos dentro de la lista la manera en la que calculamos cada elemento a partir de un bucle for:
