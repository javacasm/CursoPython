...
* **Ordenación**: podemos ordenar una lista con *mi_lista.sort()*. Una vez ordenada sabremos que el primero o el último serán el mínimo o el máximo valor.
    - Podemos ordenarla en orden inverso con *mi_lista.sort(reverse=True)* para .  Conviene recalcar que la operación se realiza sobre la propia lista, no es que la llamada nos devuelva una lista ordenada. 
...
['hola', 'Hola', 'hOLA', 'ola']
```    
- Para que no dependa de si la primera letra es mayúscula usaremos *str.capitaliza* que usa el primer carácter de cada cadena como mayúscula
```python
>>> palabras = ['Hola','ola','hola','hOLA']
